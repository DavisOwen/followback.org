...
class Getter:
    ...
    def login(self, force = False):
        if (not self.isLoggedIn or force):
            ...
            req = self.SendRequest('si/fetch_headers/?challenge_type=signup&guid=' + self.generateUUID(False), None, True)
            if req.status_code == 200:
                ...
                req = self.SendRequest('accounts/login/', self.generateSignature(json.dumps(data)), True)
                if req.status_code == 200:
    ...
    def getTotalFollowers(self,usernameId):
        ...
        next_max_id = ''
        while 1:
            work = self.getUserFollowers(usernameId,next_max_id)
            if work:
              temp = self.LastJson
              num = 0
