...
class InstaBot:
    ...
    def login(self):
        ...
        def handle_checkpoint(login):
            try:
                checkpoint_url = login.json()
                ...
                security_code = raw_input("Enter code: ")
                checkpoint = self.s.post(checkpoint_url,
        ...
        if login.status_code == 200:
            ...
            self.write_log('Login error! Might be a checkpoint!')
            checkpoint = handle_checkpoint(login)
            if checkpoint:
                r = self.s.get("https://www.instagram.com/")
                finder = r.text.find(self.user_login)
                if finder != -1:
                ...
                self.login_status = False
                self.write_log('Checkpoint failed!')
